...
> 관련 문서: `databaseSchema.md`, `decisionLog.md`, `agentGuide.md`, `implementationRoadmap.md`
...
- 츠쿠야 검증 체크리스트 11개 → `prompts/jp_tsukuya_prompt.md` 또는 추후 `validationChecklist.md`
- API 엔드포인트 → `apiEndpoints.md`
- 구현 일정 → `implementationRoadmap.md`
...
| 8 | 재현 | `data/generated/{level}_grammar_chunks.json` | JSON 배열 (전체) |
...
| 적재 | 재현 | `data/curated/{level}_grammar_chunks.json` | **확정본** (적재 성공 후) |
...
`decisionLog.md` 2026-04-10 항목 참조.
...
별도 `docs/validationChecklist.md`로 분리 예정 (`agentGuide.md` 미결).
...
상세는 `databaseSchema.md` 13장 동기화 흐름 참조.
...
`grammar_chunks.metadata` JSONB에 버전 정보 기록:
...
전체 N5/N4/N3 데이터셋의 큰 변경은 `decisionLog.md`에 기록:
...
- **`llm_response_cache` 테이블 적극 활용** (`databaseSchema.md` 14장)
